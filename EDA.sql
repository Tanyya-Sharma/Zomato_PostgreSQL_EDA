...
Write a query to find the top 5 most frequently ordered dishes by the customer "Arjun Mehta" in
the last 2 year. */
SELECT customer_id, customer_name, dishes, order_count FROM
(SELECT c.customer_id, c.customer_name, o.order_item dishes, COUNT(*) order_count, DENSE_RANK() OVER(ORDER BY COUNT(*) DESC) rank FROM customers c
...
(SELECT c.customer_id, c.customer_name, SUM(total_amount) total_amt_spent FROM orders o
JOIN customers c ON c.customer_id = o.customer_id
GROUP BY 1
HAVING SUM(total_amount) >= 100000
ORDER BY 1)
SELECT customer_id, customer_name FROM High_Value_Customers;

/* Q5. Orders Without Delivery
Question:
Write a query to find orders that were placed but not delivered.
Return: restaurant_name, city, and the number of not delivered orders. */
WITH Orders_Without_Delivery AS
(SELECT o.order_id, r.restaurant_name, r.city, d.delivery_status FROM orders o
JOIN deliveries d ON o.order_id = d.order_id
JOIN restaurants r ON o.restaurant_id = r.restaurant_id
WHERE d.delivery_status = 'Not Delivered')
SELECT restaurant_name, city ,COUNT(delivery_status) not_delivered_orders_count FROM Orders_Without_Delivery
GROUP BY 1,2;
...
Return: restaurant_name, total_revenue, and their rank within their city. */
With Restaurant_Revenue_Ranking AS
(SELECT restaurant_name, SUM(total_amount) total_revenue, r.city FROM orders o 
JOIN restaurants r ON r.restaurant_id = o.restaurant_id
WHERE order_date >= CURRENT_DATE - INTERVAL '2 YEAR'
GROUP BY 1,3)
SELECT restaurant_name, total_revenue, city, RANK() OVER (PARTITION BY city ORDER BY total_revenue DESC) city_rank 
FROM Restaurant_Revenue_Ranking;

/* Q7. Most Popular Dish by City
Question:
Identify the most popular dish in each city based on the number of orders. */
SELECT most_popular_dish, city, order_count FROM
...
SELECT 
    rider_id, 
	ROUND(SUM( rider_earnings)::numeric,2) total_monthly_earnings
FROM (
SELECT d.rider_id, o.total_amount * 8/100 rider_earnings , TO_CHAR(o.order_date, 'mm-yy') monthly FROM orders o 
JOIN deliveries d ON o.order_id = d.order_id
ORDER BY 1)
GROUP BY 1
ORDER BY 1;
...
Question:
Calculate the total revenue generated by each customer over all their orders. */
-- Further we can add High-Value-Customers who generated revenue with more than 10,000
SELECT 
   customer_id, 
